...
function Brand({ footer = false }) {
  return (
    <a href="#top" className="inline-flex items-center gap-3" aria-label="SportsSphere home">
      <img
        ...
        <div className="flex h-[68px] items-center justify-between gap-4">
          ...
          <div className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-1 xl:flex">
            {links.map(([label, href]) => (
              <a key={label} href={href} className="rounded-full px-4 py-2 text-sm font-medium text-slate-600 transition hover:bg-slate-100 hover:text-sky-700">
                {label}
              </a>
            ))}
          </div>
          ...
          <div className="hidden shrink-0 items-center gap-2 xl:flex">
            <a href="#signin" className="rounded-full px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-100">Sign in</a>
            <a href="#features" className="flex items-center gap-3 rounded-full bg-[#082b58] py-2 pl-5 pr-2 text-sm font-semibold text-white shadow-lg shadow-slate-900/15 transition hover:bg-sky-800">
              Get started
          ...
          <div id="mobile-menu" className="border-t border-slate-200 pb-4 pt-3 xl:hidden">
            <div className="grid grid-cols-2 gap-2">
              {links.slice(0, 4).map(([label, href]) => (
                <a key={label} href={href} onClick={() => setMenuOpen(false)} className="rounded-xl px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-100">
                  {label}
                </a>
              ))}
            </div>
            <div className="mt-3 grid grid-cols-2 gap-2 border-t border-slate-200 pt-3">
              <a href="#signin" className="rounded-xl border border-slate-200 px-4 py-3 text-center text-sm font-semibold text-slate-700">Sign in</a>
              <a href="#features" onClick={() => setMenuOpen(false)} className="rounded-xl bg-[#082b58] px-4 py-3 text-center text-sm font-semibold text-white">Get started</a>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="relative z-10 mx-auto grid min-h-[calc(100vh-89px)] max-w-[1440px] grid-cols-1 items-center gap-10 px-6 pb-12 pt-10 md:px-10 md:pb-10 md:pt-8 lg:grid-cols-12">
        <div className="flex flex-col gap-5 pt-8 md:gap-3 lg:col-span-7 lg:pt-0 xl:col-span-6">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3 md:gap-4">
            <a href="#court-booking" className="group flex items-center gap-3 rounded-full bg-sky-400 py-2 pl-6 pr-2 text-sm font-semibold text-white shadow-xl shadow-sky-950/25 transition hover:bg-sky-300 md:gap-5 md:pl-7">
              Find a court
        ...
        <div className="hidden flex-col gap-4 lg:col-span-4 lg:col-start-9 lg:mb-8 lg:flex lg:self-end xl:col-span-3 xl:col-start-10">
          <div className="hero-card ml-auto w-full max-w-sm rounded-3xl p-5 md:p-6">
            ...
            <a href="#features" className="block w-full rounded-xl bg-sky-400 py-2.5 text-center text-sm font-semibold text-white transition hover:bg-sky-300">Find Match →</a>
          </div>
          <div className="hero-card ml-auto flex w-full max-w-sm items-center gap-4 rounded-3xl p-4 md:p-5">
...
function PhotoCard({ id, number, imageName, alt, badge, badgeClass, icon, title, description, action, imagePosition = 'center' }) {
  return (
    <article id={id} className="feature-card group relative min-h-[472px] overflow-hidden rounded-[2rem] bg-slate-900 shadow-sm scroll-mt-24 lg:col-span-2">
      ...
      <div className="relative z-10 flex min-h-[472px] flex-col justify-between p-6">
        ...
        <div>
          ...
          <a href="#signin" className="group/action flex w-full items-center justify-between border-t border-white/20 pt-4 text-sm font-semibold text-white">
    ...
    <section id="features" className="relative overflow-hidden bg-slate-50 px-6 py-24 scroll-mt-24 md:px-16">
      ...
      <div className="relative z-10 mx-auto grid max-w-6xl grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-6">
        <PhotoCard
          ...
          badge="Instant booking" badgeClass="border-sky-300/30 bg-sky-400/20 text-sky-200" icon="◫"
          title="Book the right court." description="Explore nearby courts, compare live availability, and reserve your ideal time slot in a few taps." action="Browse courts"
        />
        <PhotoCard
          ...
          title="Train with confidence." description="Discover certified coaches, review their expertise, and book private or group sessions around your schedule." action="Meet the coaches"
        />
        ...
        <article id="tournaments" className="feature-card group flex overflow-hidden rounded-[2rem] bg-slate-900 shadow-sm scroll-mt-24 md:col-span-2 lg:col-span-2 lg:flex-col">
          ...
          <div className="flex flex-1 flex-col justify-between p-6">
            ...
            <a href="#signin" className="mt-6 block w-full rounded-xl bg-sky-400 py-2.5 text-center text-sm font-semibold text-white transition hover:bg-sky-300">Create Tournament →</a>
          </div>
        </article>
        ...
        <article className="feature-card group relative min-h-[320px] overflow-hidden rounded-[2rem] bg-slate-900 shadow-sm md:col-span-2 lg:col-span-4">
          ...
          <div className="relative z-10 flex h-full max-w-md flex-col justify-between p-7 md:p-9">
            ...
            <a href="#signin" className="mt-8 w-fit rounded-full bg-white px-5 py-2.5 text-sm font-semibold text-slate-900 transition hover:bg-sky-100">Find a sparring partner →</a>
          </div>
        </article>
        ...
        <article className="feature-card relative min-h-[320px] overflow-hidden rounded-[2rem] bg-gradient-to-br from-sky-950 to-slate-900 p-6 shadow-sm md:col-span-2 lg:col-span-2">
          ...
          <div className="glass-dark relative z-10 mt-5 flex w-fit items-center gap-2 rounded-full px-3 py-2 text-xs font-medium text-sky-300"><span className="ping-slow h-2 w-2 rounded-full bg-emerald-400" />Ask AI anytime</div>
        </article>
      </div>
    </section>
  )
      ...
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-16 lg:grid-cols-2">
        ...
        <div id="signin" className="relative flex min-h-[430px] flex-col justify-between overflow-hidden rounded-[2rem] bg-slate-950 p-10 scroll-mt-24">
          ...
          <div className="relative z-10">
            <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-full border border-white/10 bg-white/10 text-sky-400">✦</div>
            ...
            <form className="mt-8 flex items-center gap-2 rounded-full border border-white/10 bg-white/5 p-1.5" onSubmit={(event) => event.preventDefault()}>
              <input type="email" required placeholder="Your email address" aria-label="Your email address" className="w-full bg-transparent px-4 text-sm text-white outline-none placeholder:text-white/35" />
              <button className="shrink-0 rounded-full bg-sky-400 px-5 py-2 text-xs font-semibold text-white transition hover:bg-sky-300">Join free</button>
            </form>
            <p className="mt-4 text-xs text-white/30">No credit card required. Free account forever.</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <footer className="relative overflow-hidden bg-[#03142f] text-white">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 pb-8 pt-16 md:px-10 md:pt-20">
        <div className="grid grid-cols-2 gap-x-6 gap-y-12 pb-14 lg:grid-cols-12 lg:gap-10">
          <div className="col-span-2 lg:col-span-5">
            ...
            <div className="mt-7 flex items-center gap-3">
              <SocialIcon label="Instagram">◎</SocialIcon><SocialIcon label="Facebook">f</SocialIcon><SocialIcon label="YouTube">▶</SocialIcon>
            </div>
          </div>
          <FooterLinks title="Platform" links={[['Book courts', '#court-booking'], ['Find coaches', '#coach-booking'], ['Tournaments', '#tournaments'], ['Matchmaking', '#features']]} />
          <FooterLinks title="Support" links={[['How it works', '#how-it-works'], ['Help centre', '#help'], ['Contact us', '#contact'], ['Safety', '#safety']]} />
        ...
        <div className="flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-7 md:flex-row">
          <p className="text-xs text-white/35">© 2026 SportsSphere. All rights reserved.</p>
          <div className="flex items-center gap-6">{['Privacy', 'Terms', 'Cookies'].map((item) => <a key={item} href={`#${item.toLowerCase()}`} className="text-xs text-white/35 transition hover:text-white">{item}</a>)}</div>
        </div>
      </div>
    </footer>
  )
...
function FooterLinks({ title, links }) {
  return (
    <div className="col-span-1 lg:col-span-2">
      ...
      <div className="flex flex-col gap-3.5">{links.map(([label, href]) => <a key={label} href={href} className="text-sm text-white/50 transition hover:text-sky-300">{label}</a>)}</div>
    </div>
  )
}
